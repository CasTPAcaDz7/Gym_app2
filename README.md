...
### v2.14.5 - 現代化個人檔案設計（參考用戶圖片）
- **重新設計頂部個人檔案**：參考用戶提供的圖片，實現更現代化的設計風格
- **進度環顯示**：添加圓形進度環，顯示用戶健身進度百分比（53%）
- **用戶等級標識**：在用戶名旁添加皇冠圖標，表示用戶等級或身份
- **認證狀態系統**：顯示用戶認證狀態（已認證/未認證），配合圖標指示
- **優化視覺佈局**：
  * 左側：圓形頭像配進度環和百分比標籤
  * 右側：用戶名、等級標識、認證狀態、模式切換按鈕
  * 更大的頭像尺寸（70x70），增強視覺效果
  * 青色主題進度環與應用整體配色一致
- **改進模式切換**：更完整的按鈕文字（學生模式/教練模式）
